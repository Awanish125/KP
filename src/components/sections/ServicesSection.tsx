...
export function ServicesSection() {
  ...
  return (
    <div ref={sectionRef} className="h-full flex items-center pl-12 lg:pl-20">
      {/* Content panel — left side, billboard visible on the right in the camera frame */}
      <div className="w-full max-w-sm lg:max-w-md">
        ...
        <h2 className="text-4xl lg:text-5xl font-extralight leading-[1.1] text-white mb-10">
          Advertising<br />
          <em className="not-italic text-white/40">Solutions</em>
        </h2>
        ...
        <ul className="space-y-0 divide-y divide-white/[0.06]">
          {SERVICES.map((s, i) => (
            <li key={s.num}>
              ...
              >
                <span className="text-[10px] font-mono text-white/20 mt-0.5 flex-shrink-0 w-5">
                  {s.num}
                </span>
                <div>
                  <p className="text-sm font-light text-white/80 group-hover:text-white transition-colors duration-200">
                    {s.title}
                  </p>
